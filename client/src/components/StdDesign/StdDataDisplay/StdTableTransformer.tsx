...
export function mask(maskObj: any): (args: CustomRenderProps) => JSX.Element {
  return (args: CustomRenderProps) => {
    ...
    if (typeof maskObj?.[args.text] === "function") v = maskObj[args.text]();
    else if (typeof maskObj?.[args.text] === "string") v = maskObj[args.text];
    else v = args.text;
...
export function longTextWithEllipsis(len: number): (args: CustomRenderProps) => JSX.Element {
  return (args: CustomRenderProps) => {
    if (args.isExport || args.isDetail) return args.text;
...
export function maskRenderWithColor(maskObj: any) {
  return (args: CustomRenderProps) => {
    let label: string;
    if (typeof maskObj[args.text] === "function") label = maskObj[args.text]();
    else if (typeof maskObj[args.text] === "string") label = maskObj[args.text];
    else label = args.text;

    if (args.isExport) return label;
    ...
    return args.text ? h(Tag, { color: colorMap[args.text as keyof typeof colorMap] }, maskObj[args.text]) : "-";
  };
}
