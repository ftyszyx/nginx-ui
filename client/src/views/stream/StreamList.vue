...
function handleAddStream() {
  stream
    .save(name.value, { name: name.value, content: "server\t{\n\n}" })
    .then(() => {
  ...
  <ACard :title="$gettext('Manage Streams')">
    ...
    >
      <template #actions="{ record }">
        <AButton v-if="record.enabled" type="link" size="small" @click="disable(record.name)">
          {{ $gettext("Disable") }}
        </AButton>
        <AButton v-else type="link" size="small" @click="enable(record.name)">
          {{ $gettext("Enable") }}
        </AButton>
        <ADivider type="vertical" />
        <AButton type="link" size="small" @click="handle_click_duplicate(record.name)">
          {{ $gettext("Duplicate") }}
        </AButton>
        <ADivider type="vertical" />
        <APopconfirm
        ...
        >
          <AButton type="link" size="small" :disabled="record.enabled">
            {{ $gettext("Delete") }}
          </AButton>
        </APopconfirm>
      </template>
    </StdTable>
    <AModal v-model:open="showAddStream" :title="$gettext('Add Stream')" :mask="false" @ok="handleAddStream">
    ...
    <StreamDuplicate v-model:visible="showDuplicator" :name="target" @duplicated="() => table.get_list()" />
  </ACard>
</template>
