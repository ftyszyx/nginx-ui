...
function newSSE() {
  ...
  s.onmessage = (e: SSEvent) => {
    ...
    const notificationInstance = notification as any;

    notificationInstance[typeTrans[data.type as keyof typeof typeTrans]]({
  ...
  <span class="cursor-pointer">
    ...
    >
      ...
      <template #content>
        ...
        <AList :data-source="data" class="max-h-96 overflow-scroll">
          <template #renderItem="{ item }">
            <AListItem>
              <template #actions>
                <span key="list-loadmore-remove" class="cursor-pointer" @click="remove(item.id)">
                  <DeleteOutlined />
                </span>
              </template>
              <AListItemMeta :title="$gettext(item.title)" :description="detailRender({ text: item.details, record: item } as CustomRenderProps)">
                <template #avatar>
                  <div>
                    <CloseCircleOutlined v-if="item.type === NotificationTypeT.Error" class="text-red-500" />
                    <WarningOutlined v-else-if="item.type === NotificationTypeT.Warning" class="text-orange-400" />
                    <InfoCircleOutlined v-else-if="item.type === NotificationTypeT.Info" class="text-blue-500" />
                    <CheckCircleOutlined v-else-if="item.type === NotificationTypeT.Success" class="text-green-500" />
                  </div>
                </template>
              </AListItemMeta>
            </AListItem>
