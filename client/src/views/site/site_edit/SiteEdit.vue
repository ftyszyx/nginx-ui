...
function handle_response(r: Site) {
  if (r.advanced) advanceMode.value = true;
...
function on_mode_change(advanced: CheckedType) {
  site.advance_mode(name.value, { advanced: advanced as boolean }).then(() => {
    ...
    if (advanced) {
      ...
      return ngx
        .tokenize_config(configText.value)
        .then((r) => {
    ...
    <ACol :xs="24" :sm="24" :md="16" :lg="18">
      <ACard :bordered="false">
        <template #title>
          ...
          <ATag v-if="enabled" color="blue">
            {{ $gettext("Enabled") }}
          </ATag>
          <ATag v-else color="orange">
            {{ $gettext("Disabled") }}
          </ATag>
        </template>
        <template #extra>
          <div class="mode-switch">
            <div class="switch">
              <ASwitch size="small" :disabled="parse_error_status" :checked="advanceMode" @change="on_mode_change" />
            </div>
        ...
        <Transition name="slide-fade">
          <div v-if="advanceMode" key="advance">
            <div v-if="parse_error_status" class="parse-error-alert-wrapper">
              <AAlert :message="$gettext('Nginx Configuration Parse Error')" :description="parse_error_message" type="error" show-icon />
            </div>
            <div>
          ...
          <div v-else key="basic" class="domain-edit-container">
            <NgxConfigEditor v-model:auto-cert="auto_cert" :cert-info="certInfoMap" :enabled="enabled" @callback="save" />
          </div>
        </Transition>
      </ACard>
    </ACol>

    <ACol class="col-right" :xs="24" :sm="24" :md="8" :lg="6">
      <RightSettings />
    </ACol>
