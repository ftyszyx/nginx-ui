...
<template>
  <ACard :title="$gettext('Certificates')">
    <template #extra>
      <AButton type="link" @click="router.push('/certificates/import')">
        <CloudUploadOutlined />
        {{ $gettext("Import") }}
      </AButton>

      <AButton type="link" @click="() => refWildcard.open()">
        <SafetyCertificateOutlined />
        {{ $gettext("Issue wildcard certificate") }}
      </AButton>
    </template>
    <StdTable ref="refTable" :api="cert" :columns="certColumns" disable-view @click-edit="(id) => router.push(`/certificates/${id}`)" />
    <WildcardCertificate ref="refWildcard" @issued="() => refTable.get_list()" />
  </ACard>
</template>
