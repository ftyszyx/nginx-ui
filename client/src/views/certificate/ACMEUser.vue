...
const columns: Column[] = [
  ...
  },
  {
    ...
    customRender: (args: CustomRenderProps) => {
      if (args.text === "valid") return <Tag color="green">{$gettext("Valid")}</Tag>;
...
<template>
  <StdCurd :title="$gettext('ACME User')" :columns="columns" :api="acme_user">
    <template #edit="{ data }: { data: AcmeUser }">
      <template v-if="data.id > 0">
        ...
        <AButton type="link" @click="register(data.id, data)">
          {{ $gettext("Register") }}
        </AButton>
      </template>
    </template>
