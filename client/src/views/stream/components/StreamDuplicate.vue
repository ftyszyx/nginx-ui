...
const rulesRef = reactive({
  name: [
    {
      required: true,
      message: () => $gettext("Please input name, " + "this will be used as the filename of the new configuration!"),
    },
  ],
  target: [
    {
...
function onSubmit() {
  validate().then(async () => {
    ...
    modelRef.target.forEach((id) => {
      if (id === 0) {
        stream
          .duplicate(props.name, { name: modelRef.name })
          .then(() => {
        ...
        stream.get(props.name).then((r) => {
          stream
            .save(
              ...
              { headers: { "X-Node-ID": id } }
            )
            .then(() => {
              notification.success({
                message: $gettext("Duplicate successfully"),
                description: $gettext("Duplicate %{conf_name} to %{node_name} successfully", { conf_name: props.name, node_name: node_map[id] }),
              });
            })
            .catch((e) => {
              notification.error({
...
<template>
  <AModal v-model:open="show" :title="$gettext('Duplicate')" :confirm-loading="loading" :mask="false" @ok="onSubmit">
    <AForm layout="vertical">
      <AFormItem :label="$gettext('Name')" v-bind="validateInfos.name">
        <AInput v-model:value="modelRef.name" />
      </AFormItem>
      <AFormItem v-if="!settings.is_remote" :label="$gettext('Target')" v-bind="validateInfos.target">
        <NodeSelector v-model:target="modelRef.target" v-model:map="node_map" />
      </AFormItem>
    </AForm>
  </AModal>
</template>
