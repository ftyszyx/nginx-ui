...
install.get_lock().then(async (r: { lock: boolean }) => {
  if (!r.lock) await router.push("/install");
});
...
function onSubmit() {
  validate().then(async () => {
    ...
    await auth
      .login(modelRef.username, modelRef.password, passcode.value, recoveryCode.value)
      .then(async (r) => {
...
if (route.query?.code !== undefined && route.query?.state !== undefined) {
  loading.value = true;
  auth
    .casdoor_login(route.query?.code?.toString(), route.query?.state?.toString())
    .then(async () => {
      message.success($gettext("Login successful"), 1);
        ...
        <div class="login-form">
          ...
          <AForm id="components-form-demo-normal-login">
            <template v-if="!enabled2FA">
              ...
              <AButton v-if="has_casdoor" block html-type="submit" :loading="loading" class="mb-5" @click="loginWithCasdoor">
                {{ $gettext("SSO Login") }}
              </AButton>
            </template>
            <div v-else>
            ...
            <AFormItem v-if="!enabled2FA">
              ...
              <div v-if="passkeyConfigStatus" class="flex flex-col justify-center">
                ...
                <AButton :loading="passkeyLoginLoading" @click="handlePasskeyLogin">
                  <KeyOutlined />
