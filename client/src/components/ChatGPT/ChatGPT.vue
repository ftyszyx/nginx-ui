...
async function request() {
  ...
  function apply(input: Uint8Array) {
    ...
    line?.forEach((v) => {
      ...
      if (!hasCodeBlockIndicator) hasCodeBlockIndicator = content.includes("`");
  ...
  function isCodeBlockComplete(text: string) {
    ...
    if (matches) return matches.length % 2 === 0;
    else return true;
  }
...
<template>
  <div v-if="show" class="chat-start">
    <AButton :loading="loading" @click="send">
      <Icon v-if="!loading" :component="ChatGPT_logo" />
      {{ $gettext("Ask ChatGPT for Help") }}
    </AButton>
  </div>
  <div v-else class="chatgpt-container">
    <AList class="chatgpt-log" item-layout="horizontal" :data-source="messages">
      <template #renderItem="{ item, index }">
        <AListItem>
          <AComment :author="item.role === 'assistant' ? $gettext('Assistant') : $gettext('User')">
            <template #content>
              <div v-if="item.role === 'assistant' || editing_idx !== index" v-dompurify-html="marked.parse(item.content)" class="content" />
              <AInput v-else v-model:value="item.content" style="padding: 0" :bordered="false" />
            </template>
            <template #actions>
              ...
              <span v-else-if="!loading" @click="regenerate(index)">
                {{ $gettext("Reload") }}
              </span>
            </template>
          </AComment>
    ...
    <div class="input-msg">
      <div class="control-btn">
        <ASpace v-show="!loading">
          ...
          <AButton type="text" @click="regenerate((messages?.length ?? 1) - 1)">
            {{ $gettext("Regenerate response") }}
          </AButton>
        </ASpace>
      </div>
      <ATextarea v-model:value="ask_buffer" auto-size />
      <div class="send-btn">
        <AButton size="small" type="text" :loading="loading" @click="send">
          <SendOutlined />
        </AButton>
      </div>
    </div>
